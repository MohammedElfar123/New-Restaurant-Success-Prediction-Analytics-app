...
export default function ProviderStatisticsPage() {
  ...
  const { providerType: storedProviderType, user } = useAuthStore();
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await providerClient.get("/reports/statistics");
      ...
      console.error("[Provider Statistics] API error:", err);
      setApiData(fallbackData);
    } finally {
      setLoading(false);
    }
  ...
  // Determine if top doctors section should be shown
  const showTopDoctors = useMemo(() => {
    const type = apiData?.provider_type || providerType;
    return type === "Clinic" || type === "Hospital";
  }, [apiData, providerType]);

  // Transform API data for charts
  const stats = useMemo(() => {
    if (!apiData) return null;

    const { revenue, bookings, payment_methods, top_doctors } = apiData;
  ...
  return (
    <DashboardLayout requiredUserType="provider">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {isRTL
                ? "نظرة شاملة على أدائك والإيرادات"
                : "Overview of your performance and revenue"}
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Top Doctors Section - Only for Clinic/Hospital */}
      {showTopDoctors && (
