...
export default function HospitalBookingsPage() {
  ...
  // Fetch ALL bookings and filter by Hospital type on frontend
  const fetchBookings = useCallback(async () => {
    ...
    if (result.success) {
      // Filter on frontend to ensure only Hospital type bookings are shown
      const filteredData = result.data.filter(
        (booking) => booking.provider?.type === "Hospital"
      );
  ...
  const handleViewBooking = (booking) => {
    setOpenDropdown(null);
    router.push(`/${locale}/admin/bookings/hospitals/${booking.id}`);
  };
  ...
  return (
    <DashboardLayout requiredUserType="admin">
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        {/* Table - Hospital bookings show both hospital (provider) and doctor (provider_doctor) columns */}
                    ...
                    <td className="py-4 px-4">
                      ...
                      <div className="space-y-1">
                        ...
                        {booking.category_name && (
                          <Badge className="bg-blue-50 text-blue-700 hover:bg-blue-50 border-blue-200 font-medium text-xs">
                            {booking.category_name}
                          </Badge>
                        )}
                      </div>
